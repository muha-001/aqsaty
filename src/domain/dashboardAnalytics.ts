...
export function getDashboardMetrics(database: Database, referenceDate = new Date()): DashboardMetrics {
  ...
  const totalContractValue = database.contracts.reduce((sum, contract) => sum + contract.totalAmount, 0);
  const totalPaid = database.contracts.reduce((sum, contract) => sum + paidFor(database, contract), 0);
  const totalRemaining = Math.max(0, database.contracts.reduce((sum, contract) => sum + contract.financedAmount, 0) - totalPaid);
  const schedules = database.contracts.flatMap((contract) => contract.schedule.map((schedule) => ({ contract, schedule })));
  ...
  const lateCustomers = [...customerMap.entries()].map(([customerId, value]) => ({ name: database.customers.find((customer) => customer.id === customerId)?.name || 'زبون غير معروف', ...value })).sort((a, b) => b.amount - a.amount).slice(0, 5);
  ...
  return { totalContractValue, totalPaid, totalRemaining, overdueInstallments, dueThisWeek, topProducts, lateCustomers, collectionRate, monthlyFlow };
}
